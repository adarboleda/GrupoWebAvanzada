...
function Dashboard({ cliente, onLogout, onClienteUpdate }: DashboardProps) {
  ...
  const realizarTransferencia = async (e: FormEvent) => {
    ...
    try {
      ...
      setErrorDestino(axiosError.response?.data?.mensaje || 'Error al transferir');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleModalOverlayClick = (setter: (value: boolean) => void, resetFn?: () => void) => {
    return () => {
      setter(false);
      if (resetFn) resetFn();
    };
  ...
  return (
    <div className="dashboard-layout">
      ...
      {/* ============ MODAL TRANSFERENCIA ============ */}
      {modalTransferencia && (
        <div className="modal-overlay" onClick={handleModalOverlayClick(setModalTransferencia, () => {
          setDestinatario(null);
          setCodigoDestino('');
          setErrorDestino('');
        })}>
